...
def prepare_data(authUid,
                 ...
                 save_on_fly=True,
                 **kwargs):
    """Retreive twitter data from ES or Twitter-API and transform to a form
        consumable by *compute_times*
...
def compute_times(data_dict):
    """Compute the list of best times from given data
            ...
            1. If no data is available to compute schedule, give empty response
            ...
            4. For each day-wise-slice:
                4.1 Assign rank to each tweet-object, relative to its day,
                    based on its normalized engagement score
                4.2 Slice out top 50 ranked times into a matrix
                4.3 Take the Transpose of the matrix to get the schedule data
                    for this day
    ...
    for df_i in df_daywise:
        ...
        """Assign rank to each tweet-object, relative to its day, based on its
...
def fill_incomplete_schedule(incomplete_schedule):
    """Fills in given incomplete schedule with values from default schedule
        ...
        2. For each day in schedule:
            2.1 Create entries for missing times in existing day-entries using
                default schedule
    ...
    for item in schedule['completeSchedule']:
        """Create entries for missing times in existing day-entries from
            default schedule
        """
...
def write_schedule(schedule):
    """Gets given schedule completed if not complete and calls SAVE_SCHEDULE
        API to write it.
...
def work(interval=30):
    """Contineously comsume SQS queue and work on each authUid from it with
