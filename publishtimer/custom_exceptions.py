...
class WriteScheduleFailedError(HTTPException):
    """Exception to be raised when schedule is computed successfully by the
        publishtimer but save_schedule API returns failure response.
        The response includes error code and reason as returned by the
        save_schedule API and the schedule computed by publishtimer
    """
    def __init__(self, **kwargs):
        ...
        self.description = 'Error in writing computed schedule to profile ' + \
            'because request to WriteSchedule API failed ' + \
            'with error code: ' + \
            str(self.upstream_response.status_code) + \
            '; reason: ' + \
