...
CLIENT = connections.create_connection(
            hosts=[h.strip() for h in os.environ['ES_HOSTS'].split(',')
                   if h.strip()],
            timeout=int(os.environ['ES_TIMEOUT']))


def get_es_client(enforce_new=False, retry=True):
    """Returns the singleton Elasticsearch-client object connected to ES server
        specified by environment variable ES_HOST with default timeout
        specified by environment variable ES_TIMEOUT
    """
    global CLIENT
...
def get_tweet_index():
    """Generates ES index name for tweet object from current time as
...
class Tweet(DocType):
    ...
    def save(self, **kwargs):
        ...
        try:
            ...
            raw_input("ConnectionTimeout with Elasticsearch." +
                      str(ct) +
